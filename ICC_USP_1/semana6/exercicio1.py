...
Sejam n o número de peças inicial e m o número máximo de peças que é possível retirar em uma rodada. Para garantir que o computador ganhe sempre, 
...
Com o objetivo do EP já definido, uma dúvida que deve surgir nesse momento é como modelar o jogo de forma que possa ser implementado em Python 3 
...
O programa deve implementar:
    ...
    jogada do computador de acordo com a estratégia vencedora.
    ...
    Uma função partida que não recebe nenhum parâmetro, solicita ao usuário que informe os valores de n e m e inicia o jogo, alternando entre jogadas 
    do computador e do usuário (ou seja, chamadas às duas funções anteriores). A escolha da jogada inicial deve ser feita em função da estratégia vencedora,
...
Cuidado: o corretor automático não funciona bem se você tiver alguma chamada a input() antes da definição de todas as funções do jogo 
(a menos que essa chamada esteja dentro de uma função). Se seu programa usar input() sem que ele esteja dentro de alguma função, coloque-o no final do programa.
Campeonatos
...
você deverá criar uma outra função chamada campeonato. Essa nova função deve realizar três partidas seguidas do jogo e, ao final, mostrar o placar dessas três partidas
...
def computador_escolhe_jogada(n, m):    #Uma função computador_escolhe_jogada que recebe, como parâmetros, 
...
def usuario_escolhe_jogada(n, m):       #Uma função usuario_escolhe_jogada que recebe os mesmos parâmetros, 
    ...
    jogadaUsuario = int(input("Informe sua jogada: "))  
    None

def partida():                                          #Uma função partida que não recebe nenhum parâmetro, solicita ao usuário
                                                        ...
                                                        # como dito anteriormente. A cada jogada, deve ser impresso na tela o estado atual
                                                        #  do jogo, ou seja, quantas peças foram removidas na última jogada e quantas
                                                        #  restam na mesa. Quando a última peça é removida, essa função imprime na tela 
                                                        # a mensagem "O computador ganhou!" ou "Você ganhou!" conforme o caso.

    n = int(input("Informe o numero de peças inicial: "))
    m = n - 1 
    while(n < m):
        m = int(input("Informe o numero de peças a ser retirado por jogada: "))
